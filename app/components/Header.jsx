...
const Header = ({ about_people, about_mission }) => {
  ...
  return (
    ...
    <>
      ...
      >
        <div
          className={`z-50 justify-between w-72 pt-10 items-center inset-y-0 px-5 fixed lg:w-full transition-transform duration-700 lg:flex lg:bg-background-500 lg:h-auto ${menuCollapse
                  ...
                  {item.subItems ? (
                    <div className="group capitalize relative">
                      ...
                      >
                        ...
                        <MdiChevronDown
                          className={`h-5 w-5 ml-2 transition-transform transform inline-flex lg:hidden ${activeSubmenu === index ? "rotate-180" : ""
                            }`}
                        />
                      </button>
                      <ul
                        className={`lg:absolute lg:top-11 lg:inset-x-0 transition-all lg:right-0 lg:left-auto lg:min-w-max z-50 lg:py-2 lg:bg-white rounded-md top-0 text-white lg:text-black mt-1 space-y-1 ${activeSubmenu === index ? "block z-50" : "hidden"
                          }`}
