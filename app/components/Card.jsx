export default function Card({count, total_col, total, title, date, description, link, link_text, author}) {
    ...
    return (
        <div className={`${bg_class} rounded-lg text-white px-8 py-10 relative inline-block my-3`}>
            {/* <div>{col} | {row} | {total_row} | {count}</div> */}
            <buttion className={`absolute top-3 right-3 text-xs ${button_class} px-2 py-1`}>{link_text}</buttion>
